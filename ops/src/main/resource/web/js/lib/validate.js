...
layui.define("jquery", function(exports) {
	...
	jQuery.validator.addMethod("isPlateNo", function(value, element) { 
	      var isPlateNo = /^([\\u4e00-\\u9fa5][a-zA-Z](([DF](?![IO])[A-Z0-9][0-9]{4})|([0-9]{5}[DF])))|([冀豫云辽黑湘皖鲁苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼渝京津沪新京军空海北沈兰济南广成使A-Z]{1}[a-zA-Z0-9]{5}[a-zA-Z0-9挂学警港澳领]{1})$/;  
	      return this.optional(element) || isPlateNo.test(value);
	},"请正确车牌号"); 
	...
	validate.prototype.init = function(param){
		this.param = param;
		this.val = $("#"+param.id).validate({
			rules:param.rules,
			messages:param.messages, 
			focusInvalid: false,
			onfocusout: function(element) { $(element).valid();},
			errorPlacement: function(error, element){
            	layui.layer.tips(error[0].innerText, element, {tips: [2, '#FF5722'], time: 2000 });
			},

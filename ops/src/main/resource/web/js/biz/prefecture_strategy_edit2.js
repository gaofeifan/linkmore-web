...
/*					dynamic_line_div=$("#dynamic_div").html();
	...
	$('#submit-button').bind('click',function(){
		...
		pageData.jsonStrategyGroup=JSON.stringify(strategyGroupArray);
		
		var reqPageData=new Object();
		reqPageData.id=pageData.id;
		reqPageData.name=pageData.name;
		reqPageData.detail=pageData.detail;
		reqPageData.jsonLockTime=pageData.jsonLockTime;
		reqPageData.jsonStrategyGroup=pageData.jsonStrategyGroup;
		//console.log(JSON.stringify(reqPageData,null,4));
		//console.log(JSON.stringify(pageData,null,4));
		
		layui.common.ajax({
			url:baseUrl+'update',
			//contentType:'application/x-www-form-urlencoded; charset=utf-8',
			//data:JSON.stringify(pageData1),
			data:reqPageData,

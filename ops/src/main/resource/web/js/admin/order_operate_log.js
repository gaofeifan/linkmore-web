...
layui.use(['layer','msg','form', 'common','datatable','laydate'], function() {
	...
	var addServerParams = function(data){  
		...
		if(searchOrder!=''){
			filter = new Object();
			filter.property = 'orderNo';
			filter.value = '%'+searchOrder+'%';
			filters.push(filter);
		} 
		var searchStallName = $('#search-stallName').val();
		if(searchStallName!=''){
			filter = new Object();
	...
	var datatable = layui.datatable.init({
		id:'order-operate-table',
		url:'/admin/admin/order_operate/list', 
		key:'id',
		columns:[ 
			...
			},
			{
				sTitle: '操作',
	          	...
	          	mRender:function(mData,type,full){
	          		var html = '';
	          		switch(mData){
	          			case 1:html += '<label style="color:green;">挂起</label>';break;
		          		case 2:html += '<label style="color:red;">关闭</label>';break;
		          		case 3:html += '<label style="color:red;">释放</label>';break;
	          		}
	          		return html;
	          	}
			},
    ...
    var operation_log = function showData(logId,callback){
    	layui.common.ajax({
			url:'/admin/admin/order_operate/detail',
    ...
    var detailInit = function(validate,lindex){
    	operation_log(logId,function(data){
    		var list = data;
    		layui.common.set({
    			id:'order-operate-detail-form',
    			data:list[0]
    		});
    		$('#order-operate-detail-form input[name=createTime]').val(dataf(list[0].createTime));
    		$('#order-operate-detail-form select').attr('disabled',true);
    		$('#order-operate-detail-form input').attr('disabled',true);
    		$('#order-operate-detail-form textarea').attr('disabled',true);
    		form.render('select');
    	});
    	$('#order-operate-cancel-detail-button').bind('click',function(){
	...
	function operationLog(){
    	...
    	valid.id = "order-operate-detail-form";
    	param.width = 650;
	...
	$('#export-button').bind('click',function(){
		...
		var searchOrder = $('#search-order').val();
		if(searchOrder!=''){
			data.orderNo = '%'+searchOrder+'%';
		}
		var searchStallName = $('#search-stallName').val();
		if(searchStallName!=''){
			data.stallName = '%'+searchStallName+'%';
		}
		var searchSource = $('#search-source').val();
		if(searchSource!='-1'){
        ...
        var url = '/admin/admin/order_operate/export';

...
layui.use(['layer','msg','form', 'common','validate','datatable','laydate'], function() {
	...
	var tempId = null;
	 var detailInit = function(validate,lindex){
		...
		function showTempInfo(){
	    	...
	    	param.url = '../enterprise/detail.html';
	    	param.title = '详情信息'; 
	...
	var datatable = layui.datatable.init({
		id:'temp-table',
		url:'/admin/coupon_enterprise/listBusiness', 
		key:'id',
		columns:[
				...
				sTitle: '操作',
	          	mRender:function(mData,type,full){
	          		...
	          		if(full.status==0){
	          			...
	          			if(full.residualReleasePeriod >= 0){
	          				html += '<a class="operation-send" data-send-id="'+full.id+'" href="send_coupon.html?tempId='+full.id+'">发送停车券</a>&nbsp;&nbsp;&nbsp;&nbsp;'
	          			}
	          		} else if(full.status==2){
	...
	var addInit = function(validate,lindex){
		...
		layui.common.ajax({
			url:'/admin/coupon_enterprise_Item/selectByEnterpriseId',
			data:{time:new Date().getTime()}, 
			contentType:'application/json; charset=utf-8',
			success:function(list){
				couponTempItemList = list;
			},error:function(){
			}
		});
		...
		$("#add-coupon").unbind("click").bind("click",function(){
			...
			if(len < 6){
				...
				html = html.replace('<input type="hidden" name="couponItemType">','<input type="hidden" name="couponItemType" value="'+$("#couponType").val()+'">')
		...
		var couponTempItem =  new Array();
		form.on('select(enterpriseDealNumber)', function(data) {
			var type = data.value;
			$.each(couponTempItemList,function(index,item){
				if(item.dealNumber == type){
					$("#temp-add-form input[name='name']").val(item.name);
					couponTempItem.push(item);
				}
			});
		});
		form.on('select(lj-hidden-sel)', function(data) {
			$.each(couponTempItem,function(index,item){
				var cycleTimeHtml = data.othis.parent().parent().parent();
				var hideItemType = cycleTimeHtml.find($("input[name='couponItemType']")).val();
				if(item.source == 1 && item.type == hideItemType &&
						item.faceAmount == data.value){
					cycleTimeHtml.find("input[name='couponValidDay']").val(item.validDay);
				}
				})
		});
		form.on('select(mz-hidden-sel)', function(data) {
			$.each(couponTempItem,function(index,item){
				var cycleTimeHtml = data.othis.parent().parent().parent();
				var hideItemType = cycleTimeHtml.find($("input[name='couponItemType']")).val();
				if(item.source == 1 && item.type == hideItemType &&
						item.conditionAmount == data.value){
					cycleTimeHtml.find("input[name='couponValidDay']").val(item.validDay);
					cycleTimeHtml.find("input[name='mj_faceAmount']").val(item.faceAmount);
				}
			});
		});
		form.on('select(zk-hidden-sel)', function(data) {
			$.each(couponTempItem,function(index,item){
				var cycleTimeHtml = data.othis.parent().parent().parent();
				var hideItemType = cycleTimeHtml.find($("input[name='couponItemType']")).val();
				if(item.source == 1 && item.type == hideItemType &&
						item.faceAmount == data.value){
					cycleTimeHtml.find("input[name='item_discount']").val(item.discount);
					cycleTimeHtml.find("input[name='couponValidDay']").val(item.validDay);
				}
			});
		});
		form.on('select(sourceId)', function(data) {
			var cycleTimeDiv = data.othis.parent().parent().parent();
			var hideItemType = cycleTimeDiv.find($("input[name='couponItemType']")).val();
			if(data.value == 0){
				if(hideItemType == 0){
					cycleTimeDiv.children("div[id='lj-id']").children("div[id='lj-id-ca-inp']").show();
					cycleTimeDiv.children("div[id='lj-id']").children("div[id='lj-id-ca-sel']").hide();
					
				}else if(hideItemType == 1){
					cycleTimeDiv.children("div[id='mz-id']").children("div[id='mz-id-ca-inp']").show();
					cycleTimeDiv.children("div[id='mz-id']").children("div[id='mz-id-ca-sel']").hide();
					
				}else if(hideItemType == 2){
					cycleTimeDiv.children("div[id='zk-id']").children("div[id='zk-id-ca-inp']").show();
					cycleTimeDiv.children("div[id='zk-id']").children("div[id='zk-id-ca-sel']").hide();
				}
			}else{
				if(hideItemType == 0){
					cycleTimeDiv.children("div[id='lj-id']").children("div[id='lj-id-ca-inp']").hide();
					cycleTimeDiv.children("div[id='lj-id']").children("div[id='lj-id-ca-sel']").show();
				}else if(hideItemType == 1){
					cycleTimeDiv.children("div[id='mz-id']").children("div[id='mz-id-ca-inp']").hide();
					cycleTimeDiv.children("div[id='mz-id']").children("div[id='mz-id-ca-sel']").show();
				}else if(hideItemType == 2){
					cycleTimeDiv.children("div[id='zk-id']").children("div[id='zk-id-ca-inp']").hide();
					cycleTimeDiv.children("div[id='zk-id']").children("div[id='zk-id-ca-sel']").show();
				}
			}
			if(data.value == 1){
				var faceAmountHtml ='';
				var validVal = null;
				var mjFaceAmountVal = null;
				var itemDiscount = null;
				var falg = true;
				$.each(couponTempItem,function(index,item){
					if(item.source == 1 && item.type == hideItemType){
						if(hideItemType == 1){
							faceAmountHtml += '<option value="'+item.conditionAmount+'">';
							faceAmountHtml += item.conditionAmount;
						}else{
							faceAmountHtml += '<option value="'+item.faceAmount+'">';
							faceAmountHtml += item.faceAmount;
						}
						faceAmountHtml += '</option>';
						if(falg && data.value == 1){
							validVal = item.validDay;
							mjFaceAmountVal = item.faceAmount;
				...
				if(hideItemType == 0){
					cycleTimeDiv.children("div[id='lj-id']").children("div[id='lj-id-ca-sel']").find('#lj-id-ca-sel-opt').html(faceAmountHtml);
					form.render('select');
				}else if(hideItemType == 1){
					cycleTimeDiv.children("div[id='mz-id']").children("div[id='mz-id-ca-sel']").children('#mz-id-ca-sel-opt').html(faceAmountHtml);
					form.render('select');
					cycleTimeDiv.find($("input[name='mj_faceAmount']")).attr("disabled",true);
					cycleTimeDiv.find($("input[name='mj_faceAmount']")).val(mjFaceAmountVal);
				}else if(hideItemType == 2){
					cycleTimeDiv.children("div[id='zk-id']").children("div[id='zk-id-ca-sel']").children('select[id="zk-id-ca-sel-opt"]').html(faceAmountHtml);
					form.render('select');
					cycleTimeDiv.find($("input[name='item_discount']")).attr("disabled",true);
					cycleTimeDiv.find($("input[name='item_discount']")).val(itemDiscount);
				}
				cycleTimeDiv.find($("input[name='couponValidDay']")).attr("disabled",true); 
				if(validVal != '' || validVal != null){
					cycleTimeDiv.find($("input[name='couponValidDay']")).val(validVal);
				}
			}else{
				if(hideItemType == 1){
					cycleTimeDiv.find($("input[name='mj_faceAmount']")).val("");
					cycleTimeDiv.find($("input[name='mj_faceAmount']")).removeAttr("disabled");
					cycleTimeDiv.find($("input[name='conditionAmount']")).val("");
				}else if(hideItemType == 0){
					cycleTimeDiv.find($("input[name='faceAmount']")).val("");
				}else if(hideItemType == 2){
					cycleTimeDiv.find($("input[name='zk_faceAmount']")).val("");
					cycleTimeDiv.find($("input[name='item_discount']")).val("");
					cycleTimeDiv.find($("input[name='item_discount']")).removeAttr("disabled");
				}
				cycleTimeDiv.find($("input[name='couponValidDay']")).removeAttr("disabled"); 
				cycleTimeDiv.find($("input[name='couponValidDay']")).val("");
			}
			cycleTimeDiv.find($("input[name='quantity']")).val("");
		});
		...
		$('#temp-add-button').bind('click',function(){
        	if(validate.valid()){
        		...
        		for(var i=1;i<len;i++){
        			var obj = new Object();
        			obj.couponItemType = $("input[name='couponItemType']").eq(i).val();
        			$("select[name='source'] option:selected").each(function(index){
        				if(i == index){
        					obj.source = $(this).val();
        				}
        			});
        			$("select[name='faceAmount_se']").each(function(index){
        				if(i == index){
        					obj.faceAmount_se = $(this).val();
        				}
        			});
        			$("select[name='conditionAmount_se'] ").each(function(index){
        				if(i == index){
        					obj.conditionAmount_se = $(this).val();
        				}
        			});
        			$("select[name='zk_faceAmount_se'] ").each(function(index){
        				if(i == index){
        					obj.zk_faceAmount_se =  $(this).val();
        				}
        			});
        			...
        			if(obj.conditionAmount_se != null && obj.conditionAmount_se != ''){
        				if(parseFloat(obj.conditionAmount_se) < parseFloat(obj.mj_faceAmount)){
        					layui.msg.tips('减免金额不得大于满足金额!');
            				return;
        				}
        			}
        			if( obj.conditionAmount != '' && obj.conditionAmount != null){
        		...
        		layui.common.ajax({
        			url:'/admin/coupon_enterprise/saveBusiness',

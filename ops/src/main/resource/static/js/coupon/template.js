...
layui.use(['layer','msg','form', 'common','validate','datatable','laydate'], function() {
    ...
    var operation_log = function showData(logId,callback){
    	layui.common.ajax({
			url:'/admin/coupon_template/detail',
    ...
    var editInit = function(validate,lindex){
    	...
    	operation_log(tempId,function(data){
    		...
    		$("#discount").html(template.discount);
    	...
    	$('#temp-edit-button').bind('click',function(){
        	if(validate.valid()){
        		...
        		layui.common.ajax({
        			url:'/admin/coupon_template/update',
	...
	function openTemplate() {
		...
		layui.msg.confirm('您确定要启用',function(){
			layui.common.ajax({
				url:'/admin/coupon_template/start',
	...
	function stopTemplate() {
		...
		layui.msg.confirm('您确定要暂停',function(){
			layui.common.ajax({
				url:'/admin/coupon_template/stop',
				data:JSON.stringify(id),
				contentType:'application/json; charset=utf-8',
				success:function(res){
					if(res.success){  
	...
	function deleteTemplate() {
		...
		layui.msg.confirm('您确定要删除',function(){
			layui.common.ajax({
				url:'/admin/coupon_template/delete',
	...
	var datatable = layui.datatable.init({
		id:'temp-table',
		url:'/admin/coupon_template/list', 
		key:'id',
		columns:[
				...
				sTitle: '操作',
	          	mRender:function(mData,type,full){
	          		...
	          		if(full.status==0){
	          			...
	          			if(full.remainNumber > 0){
	          				html += '<a class="operation-send" data-send-id="'+full.id+'" href="send_coupon.html?tempId='+full.id+'&&type='+full.type+'">发送停车券</a>&nbsp;&nbsp;&nbsp;&nbsp;'
	          			}
	          		} else if(full.status==2){
	...
	var addInit = function(validate,lindex){
		...
		$('#temp-add-button').bind('click',function(){
        	if(validate.valid()){
        		...
        		layui.common.ajax({
        			url:'/admin/coupon_template/save',
